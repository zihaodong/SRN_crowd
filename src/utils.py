...
def compute_squ_err(pred, gt):
    return np.square(np.sum(pred[:])-np.sum(gt[:]))
...
def get_data_list(data_root, mode='train'):
    ...
    if mode == 'train':
        imagepath = os.path.join(data_root, 'train')
        gtpath = os.path.join(data_root, 'train_den')

    elif mode == 'valid':
        imagepath = os.path.join(data_root, 'val')
        gtpath = os.path.join(data_root, 'val_den')

    else:
        imagepath = os.path.join(data_root, 'test')
        gtpath = os.path.join(data_root, 'test_den')
    ...
    for filepath in image_list:
        ...
        gt_file_path = os.path.join(gtpath, file_id + '.mat')
...
def reshape_tensor(tensor):
    ...
    """
    r_tensor = np.reshape(tensor, newshape=(1, tensor.shape[0], tensor.shape[1], 3))
    return r_tensor
